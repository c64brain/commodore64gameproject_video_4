...
;
; This is unlikely to change, although I want to add an option for 'portrait'
; dimensons of 32 x 64 as a feature once things are working propery.
; This gives me a good sized map with some nice tricks I can pull with the math
; to keep things from bogging down, also makes both charset and map 2048 bytes,
; so copying them around in memory is very easy.
; The per-character color is important as the new collision system will be taking
; advantage of the materials system stored in the upper half of the color byte info.
...
;---------------------------------------------------------------------------------
; After 5 rewrites of this routine I'm actually fairly happy with it.
; It keeps registers intact. For the amount of work it's doing it doesn't
; overly rely on PARAM variables. If needed I could pull the variables off the
; stack and eliminate the PARAM saving altogether, at the expense of returning clean
;
